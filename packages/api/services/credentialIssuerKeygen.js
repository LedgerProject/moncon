...
const user_did = "did:example:c276e12ec21ebfeb1f712ebc6f2"
    ...
    zencode_exec(zencodeEncrypt, {data: JSON.stringify(zenData), keys: KeyPair, conf:`color=0, debug=0`})
    .then((result) => {
        console.log(result.result);        

        zencode_exec(very, {data: result.result, keys: KeyPairvery, conf:`color=0, debug=0`})
    .then((result2) => {
        console.log(result2);
    })
    .catch((error) => {
