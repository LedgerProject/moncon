...
import { KeyPair as issuerKeypar, public_key, } from "../services/credentialIssuerKeypar.js";
import { singVC, verifyVC } from "../services/credentials/contract/contracts.js";
...
router.get('/credentialIssuerVerifier', async (req, res) => {
	...
	try {
		...
		const result = await zencode_exec(verifyVC, {data: JSON.stringify(credentia), keys: public_key, conf:`color=0, debug=0`})
...
router.get('/credentials/1', async (req, res) => {
	res.status(200).send({id:"did:moncon:6bcccb8b-a828-411d-8c98-b42a81855f74"});
});
...
router.post('/credentialIssuerSignRequest', async (req, res) => {
	try {
	    ...
	    const result = await zencode_exec(singVC, {data: JSON.stringify(zenData), keys: issuerKeypar, conf:`color=0, debug=0`})    
