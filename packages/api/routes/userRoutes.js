...
router.post('/createSubscription', async (req, res) => {
  const { metadata, description, stripeAccountId, paymentId, customerId } = req.body;
...
router.post('/chargeSubscription', async (req, res) => {
  ...
  try{
      ...
      if(!subscription?.items.data[0]){
        ...
        return res.status(400).json({ error: 'Error subscription does not exist '});
...
router.post('/token', async (req, res) => {
  ...
  try{
    const token = jwt.sign({userId}, process.env.JWT_SECRET, { expiresIn: tokenLife})
