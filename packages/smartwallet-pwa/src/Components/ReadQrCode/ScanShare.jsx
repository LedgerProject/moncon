...
const ScanShare = ({ QrResponse,socket,setCredential_verified }) => {
  ...
  useEffect(() => {
    socket.current.on('validatedCredentialResponse',validatedCredentialResponse)

    return () => {
      socket.current.off('validatedCredentialResponse',validatedCredentialResponse);      
    }
  ...
  const handleClick = () => {          
    ...
    addToast('Credential sent, waiting for response, this could take some time', 
      { 
        appearance: 'success',
        autoDismiss: true,
        autoDismissTimeout: 3000 
      ...
      <Container>  
        ...
        <div className={classes.appBar}>
          <div style={{ display: "flex", justifyContent: "center" }}>
            <div className={classes.buttonBlue} onClick={handleClick}>SEND CREDENTIAL</div>
