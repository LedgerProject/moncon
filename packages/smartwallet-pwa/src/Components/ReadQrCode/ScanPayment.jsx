...
import { AMOUNT_TO_DISPLAY, LS_PAYMENT_METHOD_KEY, LS_DID_KEY, LS_CUSTOMER_KEY } from '../../Const';
...
const ScanPayment = ({ QrResponse, socket }) => {
  ...
  const [userPaymentMethod, setUserPaymentMethod] = useState(localStorage.getItem(LS_PAYMENT_METHOD_KEY));
  ...
  const handleClick = async () => {
    ...
    if(process.env.NODE_ENV == 'development'){
      ...
      console.table({metadata, description, stripeAccountId, paymentId, customerId})
    }

    const subscription = await apiService.post('user/createSubscription', {
      metadata, description, stripeAccountId, paymentId, customerId
    })
    ...
    if(process.env.NODE_ENV == 'development'){
      ...
      console.table({subscriptionId:subscription.data.subscriptionId, amount})
    }
    const result = await apiService.post('user/chargeSubscription',{
      subscriptionId:subscription.data.subscriptionId, amount
    })
 ...
 const elStyle = {
  ...
  const handleSubscribe = async () => {
    ...
    let response
    if(!customer){
      response = await apiService.post('/user/customer', {userId,paymentId: paymentMethod.id, metadata});
      if(process.env.NODE_ENV == 'development'){
      ...
      response = await apiService.post('/user/card', {card: paymentMethod.id, customer});
      if(process.env.NODE_ENV == 'development'){
    ...
    <>
      <Container style={{marginTop: '50px'}}>
        ...
        <DialogContentText  className={classes.dialogContent}>
          <Grid variant='outlined' className={classes.paymentInfo} component="span">
            <Grid container justify='center' component="span">
              <Grid container item xs={3} justify='center' alignItems='center' component="span">
                <Avatar 
                  alt="Remy Sharp" 
                  variant='square' 
                  component="span"
              ...
              <Grid container item xs={6} lg={4} component="span">
                <Typography variant='body2' color='primary' className={classes.paymentTitle} component="span">
                  {content.title} 
                </Typography>
                <Typography variant='caption' component="span" justify='left' className={classes.paymentLink}>
                    {content.url}
                </Typography>
              </Grid>
              <Grid container item xs={3} justify='center' alignItems='center' component="span">
                <Typography variant='body2' component="span"
                  color='primary' className={classes.paymentPrice}>
                  {content?.amount || 0}€
                </Typography>
              ...
              <Grid container justify='center' >
                <Paper elevation={0} className={classes.paper}>
                  <Grid container justify='center' >
                    ...
                    <Grid container xs={12} item justify='center' style={{paddingTop: '20px',}}>
                      <Typography align='center' variant='h6' className={classes.textInitialPayment}>
                        Add your card
                      </Typography>
                    </Grid>
                    ...
                    {stripeError && (
                      <Grid xs={12}>
                        <Typography color="error" variant="caption" align="center" display="block">{stripeError}</Typography>
                      </Grid>
                    )}
